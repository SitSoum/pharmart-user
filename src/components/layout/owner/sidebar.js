...
export default function Sidebar({ isOpen, onToggle }) {
  ...
  const menuItems = [
    ...
    {
      href: "/store-owner/store-profile-settings",
      label: "Store Profile",
      icon: <FiUser />,
    },
  ];
    ...
    <>
      ...
      >
        ...
        <div className="p-3 border-b border-gray-200 flex items-center justify-between bg-white relative">
          <div
            className={`flex items-center ${isOpen ? "space-x-2" : "justify-center w-full"}`}
          >
            {isOpen ? (
              <div className="flex items-center space-x-3">
                ...
                <div className="flex flex-col">
                  ...
                  <p className="text-xs text-gray-500">For Pharmacy owners</p>
                </div>
              </div>
            ) : (
              <div className="w-10 h-10 rounded-lg overflow-hidden bg-gray-100 border border-gray-200">
...
<nav className="mt-4 flex flex-col h-[calc(100vh-120px)]">
  <ul className={`space-y-1 px-3 flex-1 ${isOpen ? "flex flex-col" : "hidden lg:flex flex-col"}`}>
    {menuItems.map((item) => {
        ...
        <li key={item.href}>
          <Link
            ...
            className={`relative flex items-center transition-all duration-200 group
              ${isOpen ? "px-3 py-3 space-x-3" : "px-3 py-3 justify-center hidden lg:flex"}
              ${isActive
          ...
          >
            <span className={`text-lg ${isActive ? "text-blue-700" : "text-gray-500 group-hover:text-gray-700"}`}>
              {item.icon}
            </span>
  ...
  {/* USER & LOGOUT */}
  <div className={`border-t border-gray-200 p-4 bg-gray-50/50 ${!isOpen ? "hidden lg:block" : ""}`}>
    {user && (
