...
export default function OAuthCallback() {
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      setStep("Reading token from URL...");
      ...
      if (!access_token) {
        setStep("No token found. Redirecting to login...");
        setTimeout(() => router.replace("/login_registration"), 800);
        return;
      }
      ...
      if (sessionError) {
        setStep("Failed to set session. Redirecting...");
        setTimeout(() => router.replace("/login_registration"), 800);
        return;
      }

      setStep("Fetching user...");
      const { user, error: userError } = await supabase.auth.getUser();
      if (userError || !user) {
        setStep("Failed to get user. Redirecting...");
        setTimeout(() => router.replace("/login_registration"), 800);
        return;
      }
      ...
      <div className="flex flex-col items-center gap-4 bg-white p-8 rounded-2xl shadow-lg w-full max-w-sm text-center">
        ...
        <div className="w-full bg-gray-200 h-2 rounded-full overflow-hidden mt-4">
          <div
            ...
            style={{
              width:
                step.includes("login attempt")
                  ? "20%"
                  : step.includes("Reading token")
                  ? "35%"
                  : step.includes("Setting session")
                  ? "50%"
                  : step.includes("Fetching user")
                  ? "65%"
                  : step.includes("Verifying account")
                  ? "80%"
                  : "100%",
            }}
          />
